...
Texture::Texture(SDL_Renderer* r, std::string text, Font* font, SDL_Color color) :
	texture_(nullptr),
	renderer_(r),
	width_(0),
	height_(0),
...
void Texture::load(std::string text, Font* font, SDL_Color color)
{
	SDL_Surface* textSurface = font->renderText(text, color);
	if (!textSurface) throw SDLError("Could not create surface from font\n");

	freeTexture();
	texture_ = SDL_CreateTextureFromSurface(renderer_, textSurface);
	if (!texture_) throw SDLError("Could not create texture from surface\n");
