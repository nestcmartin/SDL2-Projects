...
void send(const Message &msg, TCPsocket sock) {
	int n = SDLNet_TCP_Send(sock, (char*) &msg, msg.size);
	if (n < msg.size)
		error();
}

Message* recieve(TCPsocket sock) {
	int n = 0;
	int m = 0;
	while (n < sizeof(msgSizeType)) {
		m = SDLNet_TCP_Recv(sock, buffer + n, sizeof(msgSizeType) - n);
		if (m <= 0)
			return nullptr;
		n = n + m;
	}

	int size = *((msgSizeType*) buffer);
		...
		if (SDLNet_CheckSockets(socketSet, SDL_MAX_UINT32) > 0) {
			...
			if (SDLNet_SocketReady(masterSocket)) {
				...
				if (j < MAX_CLIENTS) {
					...
					send(ConnectedMsg(j), client);

				} else {
					// refuse connection (message type M1)
					Message m(_CONNECTION_REFUSED);
					send(m, client);
...
void client(char *host, int port) {
	...
	Message *m = recieve(conn);

	if (m == nullptr) {
		error(); // something went wrong
	} else if (m->id == _CONNECTED) { // M0
		id = static_cast<ConnectedMsg*>(m)->clientId; // copy the identifier to id
	} else {
	...
	send(Message(_BROADCAST_REQUEST), conn);
	send(ClientState(id, states[id]), conn);
	...
	while (true) {
		if (SDLNet_CheckSockets(socketSet, 0) > 0) {
			if (SDLNet_SocketReady(conn)) {
				...
				switch (m->id) {
				case _BROADCAST_REQUEST: // M2 - we need to broadcast state
					// broadcast state (message type M4)
					send(ClientState(id, states[id]), conn);

					break;
				case _CLIENT_DISCONNECTED: // M3 - a client has disconnected
					states[static_cast<ClientDisconnectedMsg*>(m)->clientId] =
							-1;
					stateChanegd = true;
					break;
				case _CLIENT_STATE: // M4 - a client has sent a new state
					states[static_cast<ClientState*>(m)->clientId] =
							static_cast<ClientState*>(m)->state;
					stateChanegd = true;
					break;
				default:
