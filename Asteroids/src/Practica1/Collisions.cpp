...
bool Collisions::collidesWithRotation(const Vector2D o1Pos, double o1Width, double o1Height, double o1Rot, const Vector2D o2Pos, double o2Width, double o2Height, double o2Rot) 
{
	...
	Vector2D Alu = Ac + Vector2D(-o1Width / 2.0, -o1Height / 2.0).rotate(angleA);
	Vector2D Aru = Ac + Vector2D(o1Width / 2.0, -o1Height / 2.0).rotate(angleA);
	Vector2D All = Ac + Vector2D(-o1Width / 2.0, o1Height / 2.0).rotate(angleA);
	Vector2D Arl = Ac + Vector2D(o1Width / 2.0, o1Height / 2.0).rotate(angleA);

	double angleB = o2Rot;

	Vector2D Bc = o2Pos + Vector2D(o2Width / 2.0, o2Height / 2.0);

	Vector2D Blu = Bc + Vector2D(-o2Width / 2.0, -o2Height / 2.0).rotate(angleB);
	Vector2D Bru = Bc + Vector2D(o2Width / 2.0, -o2Height / 2.0).rotate(angleB);
	Vector2D Bll = Bc + Vector2D(-o2Width / 2.0, o2Height / 2.0).rotate(angleB);
	Vector2D Brl = Bc + Vector2D(o2Width / 2.0, o2Height / 2.0).rotate(angleB);
...
bool Collisions::collides(Vector2D o1Pos, double o1Width, double o1Height, Vector2D o2Pos, double o2Width, double o2Height) 
{
	...
	if (o1Pos.getX() + o1Width < o2Pos.getX() || o2Pos.getX() + o2Width < o1Pos.getX()) return false;

	// o1 completely to the top of o2, or vice versa
	if (o1Pos.getY() + o1Height < o2Pos.getY() || o2Pos.getY() + o2Height < o1Pos.getY()) return false;
